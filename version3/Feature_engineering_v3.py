...
all_data['BldgType'] = all_data['BldgType'].replace({'1Fam':1, '2fmCon':0, 'Duplex':0, 'TwnhsE':1, 'Twnhs':0})
...
X_t = pd.get_dummies(all_data.loc[0:1459, columns_for_pc])
svm.fit(X_t, pc)
pc_pred = svm.predict(X_t)

price_category = pd.DataFrame(np.zeros((all_data.shape[0],1)), columns=['pc'], index=all_data.index)
X_t = pd.get_dummies(all_data.loc[:, columns_for_pc])
pc_pred = svm.predict(X_t)
price_category[pc_pred=='pc2'] = 1
price_category[pc_pred=='pc3'] = 2
all_data = pd.concat((all_data, price_category), axis=1)

# log1p these TotalBsmtSF, Garage area, LotArea
...
# 6 Reduce the Skewness of the numeric features
...
num_data = all_data[num_features].copy()
cat_data = all_data[cat_features].copy()
from sklearn.preprocessing import PolynomialFeatures
poly = PolynomialFeatures(degree = 2, interaction_only=False, include_bias=False)
num_data = poly.fit_transform(num_data)

# Scaling
from sklearn.preprocessing import StandardScaler
sc_X = StandardScaler()
num_data = sc_X.fit_transform(num_data)
...
# 7 Get the dummy variables
combined_data_new = combined_data.copy()
combined_data_new = pd.get_dummies(combined_data_new)



# 8. Correlation
train_data_combine = pd.concat([combined_data_new.iloc[0:1458,:], 
                              pd.DataFrame(data=y_train, columns=['SalePrice'])], axis=1)
train_data_combine_cor = train_data_combine.corr()
cor_sort_train=train_data_combine_cor.sort_values(by='SalePrice', ascending = False)

impo_features = cor_sort_train.iloc[1:-1, -1].copy().abs()
impo_features = impo_features.sort_values(ascending = False).reset_index()

# 9. Separate into training and test set
X_train = combined_data_new.iloc[0:1458, :]
X_test = combined_data_new.iloc[1458:,:]

X_train.to_csv('X_train_2degree_full.csv', index = False)
X_test.to_csv('X_test_2degree_full.csv', index = False)
y_train.to_csv('y_train_2degree_full.csv', header = ['lgSalePrice'], index=False)
impo_features.to_csv('feature_correlation_to_label_2degree_full.csv', header=['features', 'correlation'], index=False)
